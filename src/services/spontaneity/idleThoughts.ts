...
export const MIN_ABSENCE_MINUTES_FOR_THOUGHT =
  MIN_ABSENCE_HOURS_FOR_THOUGHT * 60;
...
export async function generateIdleThought(
  ...
  const { content, associatedMemory, emotionalTone, idealMood, naturalIntro } =
    await generateThoughtContent(thoughtType);
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        "[IdleThoughts] Generated thought:",
        thought.id,
        thought.thoughtType
      );
    }
 ...
 */
export async function getUnsharedThoughts(): Promise<IdleThought[]> {
  try {
...
export async function detectAndMarkSharedThoughts(
  ...
  try {
    const unsharedThoughts = await getUnsharedThoughts();
    ...
    for (const thought of unsharedThoughts) {
      ...
      if (responseLower.includes(thoughtSnippet)) {
        ...
        console.log(
          `✅ [IdleThoughts] Detected and marked thought as shared: "${thought.content.slice(
            0,
            40
          )}..."`
        );
    ...
    if (markedIds.length > 0) {
      console.log(
        `💭 [IdleThoughts] Marked ${markedIds.length} thought(s) as shared`
      );
    }
 ...
 */
async function generateThoughtContent(thoughtType: IdleThoughtType): Promise<{
  content: string;
  ...
  const idealMood = template.idealMoods
    ? template.idealMoods[
        Math.floor(Math.random() * template.idealMoods.length)
      ]
    : undefined;
...
function mapRowToThought(row: any): IdleThought {
  return {
    ...
    idealConversationMood: row.ideal_conversation_mood as
      | ConversationalMood
      | undefined,
...
async function cleanupThoughts(): Promise<void> {
  try {
    ...
    if (!error && data && data.length > MAX_UNSHARED_THOUGHTS) {
      ...
      console.log(
        `[IdleThoughts] Expired ${idsToExpire.length} excess thoughts`
      );
    }
  } catch (error) {
