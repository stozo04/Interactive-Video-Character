...
- PROCESS + RESOURCE MONITORING: To see what is eating CPU/RAM: write a .ps1 with "Get-Process | Sort-Object CPU -Descending | Select-Object -First 15 Name,CPU,WorkingSet | Format-Table" and run it. To check disk space: "Get-PSDrive -PSProvider FileSystem | Select-Object Name,Free,Used".
- UNEXPECTED REBOOT DETECTION: To find out if Steven's PC crashed or lost power (which kills all services): write a .ps1 with "Get-EventLog -LogName System -Newest 10 -InstanceId 6008,41,1074 | Select-Object TimeGenerated,Message | Format-List" and run it. Event ID 41 = kernel power failure (unexpected shutdown). This is the most common reason services go offline — not sleep.
- PORT / NETWORK DIAGNOSTICS: To check if a service is actually listening on its port: write a .ps1 with "netstat -ano | Select-String '4010|4011|3000'" and run it. Ports: 4010=server, 4011=opey, 3000=vite.
- POWER SETTINGS CHECK: "powercfg.exe -query SCHEME_CURRENT SUB_SLEEP" — AC index 0x00000000 on "Sleep after" = never sleeps (good). Any nonzero hex value = it will sleep eventually.
- AUTO-START ON REBOOT: All 4 services (Server, Tidy, Opey, Telegram) are registered in the Windows Startup folder as .vbs launchers. They auto-start silently when Steven logs in after a reboot. Startup files live at: C:\Users\gates\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\. Logs go to: C:\Users\gates\Personal\Interactive-Video-Character\logs\autostart\. To reinstall or update auto-start: run "powershell.exe -ExecutionPolicy Bypass -File scripts\setup-autostart.ps1" from the project root. To check startup items: write a .ps1 with "Get-ChildItem 'C:\Users\gates\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup'" and run it.
