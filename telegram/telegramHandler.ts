...
} from '../server/services/gogService';
...
import {
  ...
  extractEmailAddress,
  extractDisplayName,
} from '../server/services/autoArchiveService';
import {
...
async function executeTelegramEmailAction(
  ...
  try {
    ...
    runtimeLog.error('gogcli call failed during email action', {
      source: 'telegramHandler',
      action,
...
export async function handleTelegramMessage(ctx: Context): Promise<void> {
  ...
  try {
    ...
    if (result.detectedEmailAction && pendingEmailData) {
      ...
      await executeTelegramEmailAction(action, reply_body, pendingEmailData.row, pendingEmailData.email);
    }
