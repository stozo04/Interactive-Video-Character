...
```typescript
import { log } from '../../lib/logger'; // adjust relative path
...
// Flat calls
log.info(`${LOG_PREFIX} Task created`, { taskId });
log.error(`${LOG_PREFIX} Insert failed`, { error: err.message });

// Scoped logger (preferred — auto-prefixes every message)
const scopedLog = log.scoped('MyService');
scopedLog.info('Fetching tasks');
scopedLog.error('Failed to toggle task', { taskId, error: err.message });
```
...
```
agents/tidy/
  index.ts         ← Standalone entry point (npm run tidy:dev)
...
```
agents/tidy/migrations/001_create_tidy_cron_job.sql
```
